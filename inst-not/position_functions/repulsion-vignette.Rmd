...
case passed suitable values as arguments to *x* and *y* using
`position_nudge()` from package 'ggplot2', but would need to be vectors
with different *nudge* values for each observation.
...
`method` and a model formula based on `poly()` as argument to `formula`.
