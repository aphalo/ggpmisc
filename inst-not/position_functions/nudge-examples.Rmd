...
We load all the packages used in the examples. These examples were written using
different names for the functions, so we assign them to these names.

```{r}
library(ggrepel)
position_nudge_center <- ggrepel::position_nudge_repel 
position_nudge_line  <- ggrepel::position_nudge_repel_t
```
...
should be applied. In the new funcitons this reference alters only the
