...
ggplot(data=stuff, aes(x=var, y=mean))+
  ...
  stat_poly_eq(aes(label=paste(..eq.label.., after_stat(rr.label), sep="~~~")),
...
ggplot(data=stuff, aes(x=var, y=mean, weight=n))+
  ...
  stat_poly_eq(aes(weight=NULL, label=paste(..eq.label.., after_stat(rr.label), sep="~~~")),
               formula=y~x, label.x.npc=0.8, label.y.npc=0.8,
               coef.digits=3, parse=TRUE)
...
ggplot(data=stuff, aes(x=var, y=mean))+
  ...
  stat_poly_eq(aes(weight = n, label=paste(..eq.label.., after_stat(rr.label), sep="~~~")),
               formula=y~x, label.x.npc=0.8, label.y.npc=0.8,
