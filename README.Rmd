...
Testing by `R-CMD-check.yaml` at GitHub is partly 'ggplot2'-version dependent and run only under the latest R release because of very small differences in plots and corresponding graphical "snaps" used as reference. Visual difference tests are never run by CRAN because they are "fragile" and prone to unexpectedly and spuriously fail.
