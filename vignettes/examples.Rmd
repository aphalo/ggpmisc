...
These statistics can also summarize to the console the data argument received. As the summary function can be a user defined one, this allows lots of flexibility.
...
This next chunck showing how to print the whole data frame is not run as its output is more than 100 lines long as the data set contains 100 observations.
...
Finally we show how `geom_debug()` can be used.
...
```{r}
ggplot(my.data, aes(x, y, colour = group)) + geom_point() + 
  stat_peaks(geom = "debug", summary.fun = head)
```
