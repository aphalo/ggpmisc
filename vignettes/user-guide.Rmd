...
```{r}
formula <- y ~ poly(x, 3, raw = TRUE)
ggplot(my.data, aes(x, y2, fill = block, colour = block)) +
...
```{r, eval=eval_flag}
formula <- y ~ poly(x, 3, raw = TRUE)
ggplot(my.data, aes(x, y2, colour = group.abcd)) +
  geom_point(shape = 21, size = 3) +
  stat_poly_line(formula = formula) +
...
The computed "robustness" weights from the model fit object are returned by `stat_fit_residuals()` under the name of `robustness.weights`. These can be mapped to aesthetics, in this example to `size`. (These are _posterior_ weights computed by the model fit method `rlm()`. The `prior` weights set by mapping a numeric variable to the `weight` aesthetic are returned under the name of `weights`. In a fit with `lm()` prior and posterior weights are the same, and by default equal to 1. In a fit with `rlm()` prior and posterior weights differ.)
...
ggplot(my.data, aes(x, y, colour = group)) +
  ...
  stat_fit_residuals(formula = formula,
                     method = "rlm",
                     mapping = aes(size = sqrt(after_stat(robustness.weights))),
                     alpha = 2/3)
```
