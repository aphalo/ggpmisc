...
a complete example we also replace the names of the _x_, _y_ and _color_
aesthetics.
...
ggplot(mpg, aes(displ, hwy, color = factor(cyl))) +
  ...
  annotate(geom = "rect", 
           ...
           linetype = "dotted", fill = NA, color = "black") +
  geom_point() 
```
...
ggplot(lynx, as.numeric = FALSE) + geom_line() + 
  ...
  stat_peaks(geom = "text", color = "red", vjust = -0.5, 
             check_overlap = TRUE) +
  ylim(-100, 7300)
```
...
ggplot(lynx) + geom_line() + 
  stat_peaks(color = "red") +
  stat_peaks(geom = "text", color = "red", hjust = -0.2, vjust = 0.5, 
             angle = 90, check_overlap = TRUE) +
  stat_valleys(color = "blue") +
  stat_valleys(geom = "text", color = "blue", hjust = 1.2, vjust = 0.5, 
             angle = 90, check_overlap = TRUE) +
...
ggplot(my.data, aes(x, y)) +
  geom_quadrant_lines(color = "red") +
  stat_quadrant_counts(color = "red") +
  geom_point() +
...
```{r}
ggplot(my.data, aes(x, y)) +
  geom_quadrant_lines(color = "red", pool.along = "x") +
  stat_quadrant_counts(color = "red", pool.along = "x") +
  geom_point()
```
...
ggplot(my.data, aes(x, y)) +
  geom_quadrant_lines(color = "red") +
  stat_quadrant_counts(color = "red", quadrants = c(2, 4)) +
  geom_point()
```
...
ggplot(my.data, aes(x, y)) +
  geom_point() +
  geom_smooth(method = "lm", formula = formula) +
  stat_poly_eq(aes(label =  paste(..eq.label.., "*\" with \"*", 
                                  ..rr.label.., "*\", \"*", 
                                  ..f.value.label.., "*\", and \"*",
                                  ..p.value.label.., "*\".\"",
                                  sep = "")),
...
each individual group. In the plot below the simultaneous mapping to both color
and fill aesthetics creates four groups, two with data in panel A
and the other two in panel B.

```{r}
formula <- y ~ poly(x, 3, raw = TRUE)
ggplot(my.data, aes(x, y2, color = group, fill = block)) +
  geom_point(shape = 21, size = 3) +
  geom_smooth(method = "lm", formula = formula) +
...
Volcano plots are scatter plots with some peculiarities. They are frequently used for transcriptomics and metabolomics. They are used to show _P_-values or FDR (false discovery rate) as a function of effect size, which can be either an increase or a decrease. Effect sizes are expressed as fold-changes compared to a control or reference condition. Colors are frequently used to highlight significant responses. Counts of significant increases and decreases are frequently also added.
...
and labels, while `scale_color_outcome()` provides suitable defaults for the
...
volcano_example.df %>%
  ...
  ggplot(., aes(logFC, PValue, color = outcome.fct)) +
  geom_point() +
...
A single panel quadrant plot is as easy to produce as a volcano plot using scales `scale_x_logFC()` and `scale_y_logFC()`. The data includes two different outcomes and two different log fold-change variables. See the previous section on volcano plots for details. In this example we use as shape a filled circle and map one of the outcomes to color and the other to fill, using the two matched scales `scale_color_outcome()` and `scale_fill_outcome()`.
...
quadrant_example.df %>%
  ...
  ggplot(., aes(logFC.x, logFC.y, color = outcome.x.fct, fill = outcome.y.fct)) +
  geom_quadrant_lines(linetype = "dotted") +
  stat_quadrant_counts(size = 3, color = "white") +
...
quadrant_example.df %>%
  ...
  all_quadrant_counts(size = 3, color = "white") +
  scale_x_logFC(name = "Transcript abundance for x%unit") +
  scale_y_logFC(name = "Transcript abundance for y%unit") +
  scale_color_outcome() +
  scale_fill_outcome() +
