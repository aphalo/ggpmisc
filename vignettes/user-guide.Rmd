...
```{r, eval=eval_flag}
formula <- y ~ poly(x, 3, raw = TRUE)
ggplot(my.data, aes(x, y.poly.grp, colour = group, grp.label = group)) +
  geom_point() +
  stat_poly_line(formula = formula) +
...
```{r}
formula <- y ~ poly(x, 3, raw = TRUE)
ggplot(my.data, aes(x, y.poly.grp, linetype = group, grp.label = group)) +
  geom_point() +
...
```{r}
formula <- y ~ poly(x, 3, raw = TRUE)
ggplot(my.data, aes(x, y.poly.grp, fill = block, colour = block)) +
  geom_point(shape = 21, size = 3) +
  stat_poly_line(formula = formula) +
  stat_poly_eq(aes(label = after_stat(rr.label)), size = 2.7,
               geom = "label_npc", alpha = 0.05,
               formula = formula) +
  facet_wrap(~group, scales = "free_y")
```
...
```{r, eval=eval_flag}
formula <- y ~ poly(x, 3, raw = TRUE)
ggplot(my.data, aes(x, y.poly.grp, colour = group.abcd)) +
  geom_point(shape = 21, size = 3) +
  stat_poly_line(formula = formula) +
  stat_poly_eq(aes(label = after_stat(rr.label)), size = 3, formula = formula) +
  facet_wrap(~group, scales = "free_y")
```
...
ggplot(my.data, aes(x, y.poly.grp, colour = group.abcd)) +
  geom_point(shape = 21, size = 3) +
  stat_poly_line(formula = formula) +
  stat_poly_eq(use_label("R2"), size = 3, label.y = c(0.95, 0.95, 0.9, 0.9),
               formula = formula) +
  facet_wrap(~group, scales = "free_y")
```
...
```{r, eval=eval_flag}
formula <- y ~ poly(x, 3, raw = TRUE)
ggplot(my.data, aes(x, y.poly.grp, colour = group, fill = block)) +
  geom_point(shape = 21, size = 3) +
  stat_poly_line(formula = formula) +
  stat_poly_eq(aes(label = after_stat(rr.label)), size = 3, alpha = 0.1,
...
ggplot(my.data, aes(x, y.poly.grp, colour = group)) +
  ...
  stat_poly_eq(geom = "text", aes(label = after_stat(eq.label)),
               label.x = c(10, 9), label.y = c(-0.15, 2.5), hjust = "inward",
               formula = formula)
```
...
ggplot(my.data, aes(x, y.sd3)) +
  ...
  stat_ma_line(color = "red", orientation = "y", linetype = "dashed") +
...
```{r, warning=FALSE}
ggplot(my.data, aes(x, y.poly.grp, group = group, linetype = group, 
...
When using `stat_fit_glance()`, `stat_fit_tidy()`, `stat_fit_augment()` and `stat_fit_tb()` the documentation cannot describe the variables present in the returned `data` because they call methods using their generic interface and the exact specialization used depends on the argument passed to method, which can be even one that I have not been aware of and never tested. Even in the case of other model fitting statistics like `stat_poly_eq()` which variables are present or are set to `NA` depends on the model fit `method` used and in some cases also on the arguments passed to the method.

In 'ggpmisc' (>= 1.0.0) when executed in an *interactive* R session statistics display by default a message listing either the names of variables in the returned data frame containing non-missing data or the short names of the formatted variables.
...
```{r, eval=(eval_broom && eval_gginnards), message=TRUE}
old.options <- options(ggpmisc.stat.vars.message = "colnames")
# formula <- y ~ poly(x, 3, raw = TRUE)
# broom::augment does not handle poly() correctly
formula <- y ~ x + I(x^2) + I(x^3)
ggplot(my.data, aes(x, y.poly)) +
