...
Statistics `stat_poly_line()`, `stat_ma_line()`, `stat_quant_line()` and `stat_quant_band()` return numeric data suitable for plotting lines or lines plus bands. They are similar to `ggplot2::stat_smooth()` in their use. They return a basic set of variables, `x`, `y`, `ymin,` `ymax`, `weights` and `robustness.weights` which can be expanded by passing `fm.values = TRUE`. The expanded returned `data` contains in addition `n`, `r.squared`, `adj.r.squared` and `p.value` when available. These numeric variables make it possible to conditionally hide or highlight using aesthetics, fitted lines that fulfil conditions tested on these variables. The number of rows per `group` and `panel` in the plot, i.e., number of points used to plot the smooth line, is that given by parameter `n` with default `n = 80`. The user interface and default arguments are consistent with those of statistics `stat_poly_eq()`, `stat_ma_eq()` and `stat_quant_eq()` described above.
...
We first generate a set of artificial data suitable for the plotting examples in this and subsequent sections. In these data, `x` has no error variation, and `y` has errors normally distributed with `mean = 0` and `sd = 1`, `y.grp` is `y` with a difference in mean between levels of `group`. `y.otlr` is `y` with 5 _outlier_ values with larger error variation, with `sd = 3`. `wght.otlr` is set to `0` for the outliers and to `1` otherwise and will be used as prior weights. `y.poly` are data from a third degree polynomial. `y.poly.grp` is `y.poly` with different slope and mean per group.
...
In 'ggplot2' (>= 4.0.0) `layout` controls on which panels plotting takes place. It is a parameter inherited by statistics and geometries defined in 'ggpmisc' from their parent object `layer`. For details, see the documentation of `ggplot2::layer()`. 

```{r}
ggplot(my.data, aes(x, y.grp)) +
  geom_point() +
  stat_correlation(layout = 1) +
  facet_wrap(~group)
```
...
The statistic `stat_poly_line()` adds a layer nearly identical to that from `ggplot2::stat_smooth()` but has the same user interface and default arguments as `stat_poly_eq()`. It also supports natively a wide range of model fit functions including wrapper functions that do model selection and/or method selection based on data on a per group basis.
...
formula <- y ~ poly(x, 3, raw = TRUE)
ggplot(my.data, 
       aes(x, y.poly.grp, colour = group, grp.label = group)) +
  geom_point() +
  stat_poly_line(formula = formula) +
  stat_poly_eq(aes(label = 
...
formula <- y ~ poly(x, 3, raw = TRUE)
ggplot(my.data, 
       aes(x, y.poly.grp, linetype = group, grp.label = group)) +
  geom_point() +
...
formula <- y ~ poly(x, 3, raw = TRUE)
ggplot(my.data, 
       aes(x, y.poly.grp, fill = block, colour = block)) +
...
In the plot below the simultaneous mapping to both colour and fill aesthetics creates four groups, two with data in panel A and the other two in panel B. We can override the computed positions with a vector passed as argument to `label.y` giving the `y` position (in `npc` units) four each of the four labels.

```{r, eval=eval_flag}
formula <- y ~ poly(x, 3, raw = TRUE)
ggplot(my.data, 
       aes(x, y.poly.grp, colour = group, fill = block)) +
  geom_point(shape = 21, size = 3) +
  stat_poly_line(formula = formula) +
  stat_poly_eq(aes(label = after_stat(rr.label)),
               size = 3, 
...
```{r, warning=FALSE}
ggplot(my.data, aes(x, y.poly.grp, color = group)) +
  geom_point() +
...
Package 'mixtools' implements the fitting of mixture models. Currently, 'ggpmisc' only supports fitting of mixtures of Normals with function `normalmixEM()`. Fiting of a single Normal is also supported. The fit is done numerically based on maximum likelihood with an EM or ECM algorithm.

In the case of density distributions the position of quantiles, especially near the tails is a frequent addition to plots, and these are computed in `stat_distrmix_line()` and `stat_distrmix_area()` from the cumulated density (CDF) both for the mix and for its components. The DF, CDF and quantile positions are all returned.

With the default `fullrange = TRUE` the dendity distributions are extended to the limits of the scale. In contrast, with `fullrange = FALSE` the distributions are trimmed to the range of the data.
...
Orientation is set automatically based on the mapping of the data to the `x` pf the `y` aesthetic.

```{r}
ggplot(faithful, aes(y = waiting)) +
   stat_distrmix_line() +
   stat_distrmix_eq(label.x = "right", lable.y = "bottom")
```
...
Depending on the model fit `method`, two types of _weights_ are available: prior weights given by a numeric variable mapped to `weight` aesthetic and posterior weights, actually used, and possibly computed during model fitting. Depending on the model fit `method`, prior and posterior weights can be the same or not. Function `stat_fit_residuals()` returns the _prior_ weights supplied as input in the variable named `weights` and the posterior actually used weights, possibly computed "robustness" weights, in the variable named `robustness.weights`. Both weights can be mapped to aesthetics, in the example below `robustness.weights` are mapped to `size`. In a model fit with `lm()`, prior and posterior weights are the same, and by default equal to 1. In a fit with `rlm()` prior and posterior weights differ.

```{r}
ggplot(my.data, aes(x, y.otlr, colour = group)) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  stat_fit_residuals(method = "rlm",
                     mapping = aes(size = after_stat(robustness.weights)),
                     alpha = 1/2) +
  scale_size_area(name = "Robustness\nweights", max_size = 3)
```
...
ggplot(my.data, aes(x, y.otlr, colour = group)) +
  ...
  stat_fit_residuals(method = "rlm",
                     ...
                     alpha = 1/2) +
  scale_y_continuous(name = "Robustness weights")
```
...
ggplot(my.data, aes(x, y.otlr)) +
  ...
  stat_fit_deviations(formula = formula, method = "rlm",
                      ...
                      show.legend = TRUE) +
  scale_color_gradient(name = "Robustness\nweight",
...
ggplot(my.data, aes(x, y.sdinc)) +
  ...
  stat_fit_deviations(method = "gls", 
                      method.args = list(weights = varPower(form = ~ x)),
                      mapping = 
                        aes(colour = after_stat(robustness.weights)),
                      show.legend = TRUE) +
  scale_color_gradient(name = "Power of variance\nweight",
                       low = "red", high = "blue") +
  geom_point()
```

Of course it is also possible to plot the weights themselves, most easily using `stat_fit_residuals()`.
...
ggplot(my.data, aes(x, y.sdinc)) +
  stat_fit_residuals(method = "gls", 
                     ...
                     mapping = 
                       aes(y = stage(start = y.otlr,
                                     after_stat = robustness.weights))) +
  scale_y_continuous(name = "Power of variance, fitted weight")
```
