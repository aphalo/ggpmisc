...
Volcano plots are frequently used for transcriptomics and metabolomics. They are used to show _P_-values as a function of effect size, which can be an increase or a decrease. Effect sizes are expressed as fold-changes compared to a control or reference condition. Colours are frequently used to highlight significant responses. Counts of significant increases and decreases are frequently also added.

Package 'ggpmisc' provides several building blocks that facilitate the production of volcano plots: _scales_ and helper functions. Outcomes encoded as -1, 0 or 1, as seen in the tibble below need to be converted into factors with suitable labels for levels. This can be easily done with
function `outcome2factor()`.

```{r}
head(volcano_example.df) 
```

By default the logFC data es expected as log2 transformed, but this can be
overriden as well as the use of untransformed fold-change for tick labels. 
Scale `scale_x_logFC()` in addition by default expands the scale limits to
make them symmetric around zero. If `%unit` is included in the name character
...
## Appendix: Quadrant plots

Quadrant plots are used to compare responses to two different conditions or treatments, when many
responses are measured as in transcriptomics or metabolomics. A single panel quadrant plot
is as easy to produce as a volcano plot. The data includes to different outcomes and two
different log fold-change variables. A quadrant plot is a scatter plot comparing responses to
...
To plot in separate panels those observations that are significan along both _x_ and _y_ axes, _x_ axis, _y_ axis, or none, with quadrants merged takes more effort. We first define two helper functions to add counts and quadrant to each of the four panels.
...
```{r}
    # we use Inf instead NA and rely on clipping as rows containing NAs are removed
all_quadrant_lines <- function(...) { 
  list(
    geom_quadrant_lines(data =  data.frame(outcome.xy.fct = factor(c("xy", "x", "y", "none"),
                                                                   levels = c("xy", "x", "y", "none")),
                                           xintercept = c(0, 0, Inf, Inf),
                                           yintercept = c(0, Inf, 0, Inf)),
                        xintercept = NULL, yintercept = NULL,
                        aes_(xintercept = ~xintercept, yintercept = ~yintercept),
                        ...)
  )
}
```

And use these functions to build the final plot.
