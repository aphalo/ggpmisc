...
Highlighting  1/4 of the observations by underplotting with larger black points.
...
A different way of highlighting 1/4 of the observations, using overplotting with
...
Add text labels to 1/4 of the observations. 

```{r}
ggplot(data = d, aes(x, y, label = lab, color = group)) +
  geom_point() +
  stat_dens2d_labels(keep.fraction = 0.5)
```

Use with geom `"text"` we can statically adjust the positioning of labels.
...
Using the geoms from package 'ggrepel' avoids clashes among labels or on top of data points. However the current version of these geoms draws segments for the empty text labels making this stat of limited use until 'ggrepel' is updated. One example with `text_repel` follows. 
...
Using `NA` as fill makes the observations with labels set to `NA` to be skipped completely, possibly leading to labels overlapping unlabelled observations.
