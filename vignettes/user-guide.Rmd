...
defauls. Beware, though, that what are good values, may depend on individual 
data sets even if they include the same number of observations. For the
selection of oservations to work cleanly, the argument for `n` must create a
...
A different way of highlighting the same observations. We also shift one group
...
Highlight 1/10 of observations in from lowest density areas of the plot for each
individual group.
...
results in labels being omitted.
...
Use `""` as label replacements.

```{r}
ggplot(data = d, aes(x, y, label = lab, color = group)) +
  geom_point() +
  stat_dens2d_labels(geom = "text", 
                     keep.fraction = 0.25,
                     label.fill = "", vjust = -0.3)
```

Using the geoms from package 'ggrepel' avoids clashes among labels or on top
of data points. However the current version of these geoms draws segments for 
these empty text labels making this stat of limited use. One example with 
`text_repel` follows. 
...
With `label_repel` empty boxes are also output.
