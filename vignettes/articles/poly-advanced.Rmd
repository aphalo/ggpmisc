...
Function `segreg()` makes it possible to fit LM and GLM that segmented. 
Differently to 'lspline', the knots can be fitted as well as the segment slopes
and intecept. As 'lspline' it used a model formula to set the model including
control of the segmentation. Differently to `lspline()`, `segreg()` is a model
fit function.
...
ggplot(mpg, aes(displ, hwy)) +
  ...
  stat_poly_eq(aes(label = 
                     paste("x[knot]~`=`~", 
                           round(after_stat(knots)[[1]][1, 2], 2), 
                           "*\"; L: \"*y~`=`~", 
                           round(after_stat(b_0), 2), # "+",
                           round(after_stat(b_1), 2), "~x",
                           "*\"; R: \"*y~`=`~", 
                           round(after_stat(b_0) + 
                                   after_stat(knots)[[1]][[2]] * 
...
In the next example we fix the slope of the second segment at zero to keep it
horizontal. Because of how the parameters are returned, the code to assemble
the equation needs to be rewritten.
