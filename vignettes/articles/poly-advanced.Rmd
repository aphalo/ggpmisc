...
```{r, fig.width = 7, fig.height = 5}
ggplot(subset(mpg, class == "suv"), aes(displ, hwy)) +
  stat_fit_fitted(method = MASS::rlm, colour = "red") +
  stat_fit_deviations(method = MASS::rlm, colour = "red") +
  geom_point()
```
...
```{r, fig.width = 7, fig.height = 5}
ggplot(subset(mpg, class == "suv"), aes(displ, hwy)) +
  stat_fit_residuals(method = MASS::rlm)
```
...
```{r, fig.width = 7, fig.height = 5}
ggplot(subset(mpg, class == "suv"), aes(displ, hwy)) +
  stat_fit_fitted(method = robustbase::lmrob, colour = "red") +
  stat_fit_deviations(method = robustbase::lmrob, colour = "red") +
  geom_point()
```
...
```{r, fig.width = 7, fig.height = 5}
ggplot(subset(mpg, class == "suv"), aes(displ, hwy)) +
  stat_fit_residuals(method = robustbase::lmrob)
```
...
ggplot(subset(mpg, class == "suv"), aes(displ, hwy)) +
  geom_point() +
  stat_poly_line(method = MASS::lqs) +
...
```{r, fig.width = 7, fig.height = 5}
ggplot(subset(mpg, class == "suv"), aes(displ, hwy)) +
  stat_fit_fitted(method = MASS::lqs, colour = "red") +
  stat_fit_deviations(method = MASS::lqs, colour = "red") +
  geom_point()
```
...
```{r, fig.width = 7, fig.height = 5}
ggplot(subset(mpg, class == "suv"), aes(displ, hwy)) +
  stat_fit_residuals(method = MASS::lqs)
```
...
```{r, fig.width = 7, fig.height = 5}
ggplot(subset(mpg, class == "suv"), aes(displ, hwy)) +
  stat_fit_residuals(method = robustbase::ltsReg)
```
...
Package 'lspline' provides functions that can be used to fit linear splines with function `lm()`. These functions are called from the model formula, and do not fit the change points where the knots are located. These functions, `lspline()`, `qlspline()` amd `elspline()` can be used
directly in the model formula passed to `stat_poly_line()`. They can be used also with `stat_poly_eq()` but the equation label returned is incorrect, with the terms for the different segments displayed as the terms for the powers of a polynomial. Thus, it is necessary to construct in the call to `aes()` the equations for the different segments. The other labels are correctly generated.
...
ggplot(mpg, aes(displ, hwy)) +
  geom_point() +
  stat_fit_deviations(method = "lm", formula = my.formula) +
  stat_fit_fitted(method = "lm", formula = my.formula, 
                  colour = "red", alpha = 0.5)
```
...
It is posisble to use `rlm()` instead of `lm()`.
...
Highlighting fitted deviations.

```{r, fig.width = 7, fig.height = 5}
ggplot(mpg, aes(displ, hwy)) +
  stat_fit_deviations(method = segreg, 
                      formula = y ~ seg(x, npsi = 1, est = c(1, 0)),
                      colour = "red") +
    stat_fit_fitted(method = segreg, 
                 formula = y ~ seg(x, npsi = 1, est = c(1, 0)),
...
**Note:** The lines for cubic and other smoothing splines can be added to plots
...
ggplot(mpg, aes(displ, hwy)) +
  ...
  stat_fit_fitted(method = "fit_or_not", colour = "red", size = 0.25) +
  stat_fit_deviations(method = fit_or_not, linewidth = 0.25) +
  facet_wrap(~class, ncol = 2)
```
...
```{r, fig.width = 7, fig.height = 5}
ggplot(mpg, aes(displ, hwy)) +
  stat_fit_deviations(method = lm2segmented.lm, colour = "red") +
  geom_point() +
  stat_fit_fitted(method = lm2segmented.lm, colour = "red")
```
