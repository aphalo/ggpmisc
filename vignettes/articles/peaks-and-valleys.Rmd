...
A mapping to the `label` aesthetic can be created with function `use_label()`.

```{r}
p.lx0 +
  stat_peaks(use_label("x", "y", sep = " "),
             x.label.fmt = "%Y:",
             y.label.fmt = "%i",
             geom = "text_repel",
             direction = "y",
...
It is also easy to combine _x_ and _y_ labels within a call to `aes()`. Function `paste()` or any other character string manipulation function can be used within a call to `aes()` to "edit" the text to be added to the plot.

```{r}
p.lx0 +
  stat_peaks(aes(label = after_stat(paste(x.label, y.label))),
             x.label.fmt = "%Y:",
             y.label.fmt = "%i",
             geom = "text_repel",
             direction = "y",
...
```{r}
p.lx0 +
  stat_peaks(f_use_label("y", "x", format = "%s lynx skins in year %s"),             x.label.fmt = "%Y",
             y.label.fmt = "%i",
             span = NULL,
             geom = "text_s",
             hjust = "right",
  ...
  stat_peaks(span = NULL,
             ...
             alpha = 1/5,
             colour = "red") +
  stat_valleys(f_use_label("y", "x", format = "%s lynx skins in year %s"),
               x.label.fmt = "%Y",
               y.label.fmt = "%i",
               span = NULL,
               geom = "text_s",
...
```{r}
p.lx0 +
  stat_peaks(f_use_label("y", "x", format = "%s lynx skins in year %s"),
             x.label.fmt = "%Y",
             y.label.fmt = "%i",
             span = NULL,
             geom = "text_s",
