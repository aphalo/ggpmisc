...
Layer functions in 'ggpmisc' can be used for fitting non-linear models, but given the diversity of possible model formulas, automatic generation of annotations for the model fit equations have not been implemented.

This article gives examples using the 'broom' based statistics, which have been available for several years, and tests the use of `stat_poly_eq()` and `stat_poly_line()` which in 'ggpmisc' (>= 1.0.0) are much less strict in their requirements for the model fit objects.
