...
### stat_poly_eq(), stat_poly_line() and stat_smooth_xy()
...
`lm()`, `MASS:rlm()`, `quantreg::rq()` (only median regression) or a
user-supplied model fitting function and arguments. By default this _statistic_
behaves similarly as `ggplot2::stat_smooth()` with `method = "lm"` for _y_ on
_x_ fits but adds support for model formulas of _x_ on _y_ (in addition to the
use of `orientation = "y"`). Its default behaviour is to generate labels as
character strings using R's _expression_ syntax. This stat can also output
equations using different mark-up languages, including $\LaTeX$ and Markdown, or
just numeric values selected by the argument passed to parameter `output.type`.
...
as `stat_poly_eq()` while `stat_smooth_xy()` supports both _x_ and _y_ as
explanatory variable in `formula` but has the same dynamic behaviour for
default `method` as `ggplot2::stat_smooth()`.
...
### stat_quant_eq() and stat_quantile_xy()
...
for _y_ on _x_ fits but adds support for _x_ on _y_ fits.

The statistic `stat_quant_line()` adds a layer similar to that created by
`ggplot2::stat_smooth()` including confidence bands for quantile regression, but
has the same user interface and default arguments as `stat_quant_eq()` while
`stat_quantile_xy()` supports both _x_ and _y_ as explanatory variable in
`formula` but as `ggplot2::stat_quantile()` only adds a line for each quantile.
...
argument to `formula`, or consistently with `stat_smooth()` through parameter
`orientation`.
