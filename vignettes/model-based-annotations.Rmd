...
variable in formula` and also `orientation`.

The statistic `stst_quant_band()` creates a line plus band plot that is, by
default equivalent for quantile regressio of the box of a box plot showing
quartiles plus a line for the median median regression. Which three quantiles
are fitted can be set through paramater `quantiles` but in contrasts to
