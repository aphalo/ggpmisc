...
variable in formula` and also `orientation`. Confidence intervals for the
fits to individual quantiles fulfil the same purpose as in OLS fits: providing
a boundary for the _estimated line_ computed for each individual quantile.

The statistic `stat_quant_band()` creates a line plus band plot that is, by
default equivalent for quantile regression of the box of a box plot showing
quartiles plus a line for the median median regression. In this case the 
