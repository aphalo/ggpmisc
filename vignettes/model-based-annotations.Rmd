...
parameters estimates from the model fit as an equation. `stat_poly_eq` automates
this for models of _y_ on _x_ or _x_ on _y_ fitted with function `lm`,
`MASS:rlm`, `quantreg::rq` (only median regression) or a user-supplied model
fitting function and arguments. By defaul this _statistic_ behaves similarly as
`ggplot2::stat_smooth` with `method = "lm"`. Its default behaviour is to
generate labels as character strings using R's _expression_ syntax. This stat
can also output equations using different markup languages, including $\LaTeX$
and Markdown, or just numeric values selected by the argument passed to
parameter `output.type`. Nevertheless, all examples below use expression syntax.
...
quantile regressions are fitted. This case in not supported by `stat_poly_eq()`
and we need to use `stat_quant_eq()`. This statistics is very similar to
`stat_poly_eq()` both in behavior and parameters. It supports quantile
regression of _y_ on _x_ or _x_ on _y_ fitted with function `quantreg::rq`. This
_statistic_ behaves similarly to `ggplot2::stat_quantiles` with `method = "rq"`.
...
string indicating the value of the quantile. Here we use the default value
for quantiles, equivalent to those used for the box of box plots.
...
median regression) or a user-supplied model fitting function and arguments. By
defaul this _statistic_ behaves similarly as `ggplot2::stat_smooth` with `method
= "lm"`
...
_x_ or _x_ on _y_ fitted with function `lm`, `MASS:rlm`, `quantreg::rq` (only
median regression) or a user-supplied model fitting function and arguments. This
_statistic_ behaves similarly to `ggplot2::stat_smooth` and can be used together.
...
ggplot(my.data, aes(x, y)) +
  ...
  stat_fit_deviations(formula = formula, colour = "red",
                      arrow = arrow(length = unit(0.015, "npc"), 
                                   ends = "both"))
```
When weights are used, either suplier by the user, or automatically computed 
they are returned. So this allows encoding the weights using colour. We here
use a robust regression fit will `MASS::rlm`.
