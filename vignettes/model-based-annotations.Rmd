...
Table 1 below summarizes their use and the type plot layer geometries they are most useful to combine with. The four statistics at the bottom of the table require in their use more effort as the mapping to aesthetics and conversion of numeric values into character strings must be coded in the layer function call, but on the other hand they support all model fit objects catered by package 'broom' itself or any of the packages extending 'broom' such as 'broom.mixed'.

**Table 1.** Summary of features of the statistics.
...
The returned values for `output.type` different from `numeric` are shown below. The variable mapped by default to the label aesthetics is indicated with an asterisk. The returned values for `x` and `y` are for the default computed location for the labels. The returned data frame has one row per group of observations in each panel of the plot.
...
: **Notes:** (1) Inclusion of the variable in the returned `data` depends on the argument passed to parameter `method`. **\*** Indicates the default mapping to `label` aesthetic.

Even though the number of significant digits and some other formatting can be adjusted by passing arguments, in some cases it maybe necessary for the user to do the conversion to character strings. For these special cases, the statistics can return all values as `numeric`. The values returned for `output.type` equal to `numeric` are shown below. The returned data frame has one row per group of observations in each panel of the plot.

**Table 3.** Values returned by `stat_poly_eq()` , `stat_ma_eq()` , `stat_quant_eq()` and `stat_correlation()` for `output.type` equal to `numeric`.
...
: **Note:** (1) depends on the argument passed to method.
...
Being able to label equations allows us to dispense with the use of colour, which in some cases is needed for printed output.
...
**Note:** Automatic positioning using `geom_text()` and `geom_label()` is not supported when faceting uses free scales. In this case `geom_text_npc()` and/or `geom_label_npc()` or positions supplied by the user must be used.
