...
parameters `nudge_x` and `nudge_y`.

Two new position functions supporting advanced nudging are made available by package 'ggpmisc'. While `position_nudge()` implements shifts along $x$ and $y$ axes that are the same for every observation, the new functions compute the direction of the nudging specifically for each observation.

One can think of this approach as nudging away from a focal point, a line or a curve. It is deterministic, while repulsion is aleatory. Advanced nudging works very well, but only for some patterns of observations, repulsion is more generally applicable.

### position_nudge_center()

Function `position_nudge_center()` nudges radially away from a focal point if
both *nudge_x* and *nudge_y* are passed as arguments, or towards opposite sides
of a boundary line if only one of *nudge_x* or *nudge_y* is passed an argument.
By default, the "center" is the centroid computed using `mean()`, but other
functions or numeric values can be passed to override it. When data are sparse,
such nudging may be effective in avoiding label overlaps, and achieving a
visually pleasing posiitoning.
...
By default a centroid computed with `mean()` is used as focal point. If the direction is `"split"` then only the sign of `x` and/or `y` are reversed to the left and below the focal point, respectively.
...
We can override the default center coordinates by passing numeric values to `center_x` and/or `center_y`.
...
We can override the default center coordinates by a function.
