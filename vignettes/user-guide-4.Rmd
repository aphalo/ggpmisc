...
object determine the plotting order when generating the graphical output. The
Grammar of Graphics treats the list of layers as a stack using only _push_
operations. In other words, always the most recently added layer resides at the
end of the list, and overplots all layers previously added. The functions
described in this vignette allow overriding the **normal** syntax at the cost 
of breaking the expectations of the grammar. They are, as told above, to be used 
only in exceptional cases. This notwithstanding, they are rather easy to use.
The user interface is consistent across all of them. Moreover, they are 
designed to return objects that are identical to objects created using the
**normal** Grammar of Graphics.

Function | Use                     
-------- | -------------------------------------
`delete_layers()` | delete one or more layers 
`move_layers()`   | move layers to a different position
`append_layers()` | append layers at a specific position 
`which_layers()`  | obtain the index positions of layers
`show_layers()`   | extract matched or indexed laters

Although their definition does not rely on 'ggplot2' non-exported code, they 
...
To see all the fields, one needs to use `str()`, wich we use here for a single layer.
...
We start by using `which_layers()` as it produces simply a vector of idexes into
the list of layers. The third statement is useless here, but demonstrates how
layers are selected in all the functions desribed in this documment.
...
We can also easily extract matching layers with `show_layers()`. Here one layer
...
of Graphics, `object` also accpects a list of layers as argument (no example
shown).
